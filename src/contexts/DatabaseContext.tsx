...
export const DatabaseProvider = ({children}: any) => {
  ...
  const createTables = async () => {
    try {
      ...
      await db.executeSql(
        `INSERT OR IGNORE INTO categories (id, name) VALUES (1, 'Từ vựng quan trọng')`,
      );
  ...
  const deleteWordsFromCategory = async (words: string[], categoryId: number): Promise<void> => {
    try {
      ...
      await db.executeSql(
        `
        DELETE FROM word_categories WHERE word IN (${words
          .map(() => '?')
          .join(',')}) AND category_id = ?`,
        [...words, categoryId],
      );
    } catch (error) {
      console.log(error);
  ...
  const getTodaysWord = async (): Promise<Word | undefined> => {
    try {
      ...
      // Get random word using today's date as seed
      const date = new Date();
  ...
  const getHistory = async (): Promise<Word[]> => {
    try {
      ...
      LEFT JOIN av ON av.word = word_history.word
