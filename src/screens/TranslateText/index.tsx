...
const TranslateText = ({navigation, route}: Props) => {
  ...
  const enToVi = async () => {
    try {
      ...
      setFromTo(['en', 'vi']);
      const url =
        'https://clients4.google.com/translate_a/t?&client=dict-chrome-ex&sl=en&tl=vi&tbb=1&q=';
  ...
  const viToEn = async () => {
    try {
      ...
      setFromTo(['vi', 'en']);
      const url =
        'https://clients4.google.com/translate_a/t?&client=dict-chrome-ex&sl=vi&tl=en&tbb=1&q=';
      const res = await fetch(url + text);
      const data = await res.json();
      setResult(data[0]);
      setLoading(false);
